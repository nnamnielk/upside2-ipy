...
def write_plumed(n_atom, plumedFile, use_cb):
    g = t.create_group(t.root.input.potential, 'plumedforce')

    if use_cb:
        g._v_attrs.arguments = np.array(['placement_fixed_point_only_CB'])
        g._v_attrs.n_atoms = n_atom/3;
        print "OK"
    else:
        g._v_attrs.arguments = np.array(['pos'])
        g._v_attrs.n_atoms = n_atom;

    g._v_attrs.dt = 0.009;
    g._v_attrs.kbT = 0.8;
    #g._v_attrs.plumedFile = plumedFile;
    #create_array(g, 'atomsNumber', obj=[n_atom])
    create_array(g, 'plumedFile', obj=np.array([plumedFile]))


def write_plumed_pf(n_atom, plumedFile, node_name, pos, dimension):
    g = t.create_group(t.root.input.potential, node_name)
    infer_group = t.get_node('/input/potential/infer_H_O')
    n_donor = infer_group.donors.id.shape[0]
    g._v_attrs.arguments = np.array([pos])
    g._v_attrs.n_atoms = n_donor;
    g._v_attrs.dt = 0.009;
    g._v_attrs.kbT = 0.8;
    g._v_attrs.just_print = 1;
    g._v_attrs.dimension = dimension;
    create_array(g, 'plumedFile', obj=np.array([plumedFile]))
...
def write_environment(fasta, environment_library, sc_node_name, potential_type=0, weights_number = 20):
...
def write_COM_environment(fasta, environment_library):

    n_res = len(fasta)

    with tb.open_file(environment_library) as lib:
        coverage_param = lib.root.coverage_param[:]
        restype_o = lib.root.restype_order[:]
        restype_order  = dict([(str(x),i) for i,x in enumerate(restype_o)])
        sc_COM = lib.root.sc_COM[:]


    aa_types = [ restype_order[s] for s in fasta ]
    ...
    # Place SC-COM
    pgrp = t.create_group(potential, 'placement_fixed_point_vector_only_COM')
    pgrp._v_attrs.arguments = np.array(['affine_alignment'])
    ...
    placement_data = np.zeros((20,6))
    for i in range(20):
        placement_data[i,0:3] = sc_COM[i]
        placement_data[i,3:6] = (sc_COM[i]-ref_pos[1])/vmag(sc_COM[i]-ref_pos[1])
    create_array(pgrp, 'affine_residue',  np.arange(len(fasta)))
    create_array(pgrp, 'layer_index',     np.array([restype_order[s] for s in fasta]))
    create_array(pgrp, 'placement_data',  placement_data)

    # Compute acceptor coverage of the HN
    cgrp = t.create_group(potential, 'com_coverage')
    cgrp._v_attrs.arguments = np.array(['placement_fixed_point_vector_only_CB', 'placement_fixed_point_vector_only_COM'])
    cgrp._v_attrs.num_aa_types = 20
    create_array(cgrp, 'interaction_param', coverage_param)
    # group1 is the H
    create_array(cgrp, 'index1',    np.arange(n_res))
    create_array(cgrp, 'type1',     np.arange(n_res)*0)
    create_array(cgrp, 'id1',       np.arange(n_res))
    create_array(cgrp, 'index2',    np.arange(n_res))
    create_array(cgrp, 'type2',     np.arange(n_res)*0)
    create_array(cgrp, 'id2',       np.arange(n_res))
    create_array(cgrp, 'aa_types', np.array(aa_types) )

def write_bb_environment(fasta, environment_library, sc_node_name, bb_env_fn):
...
def make_wall_spring(parser, restraint_table):
    ...
    atom_header_fields = 'atom1 atom2 distance spring_constant wall'.split()
    residue_header_fields = 'residue1 residue2 distance spring_constant wall'.split()
    ...
    grp = t.create_group(potential, 'walls')
    grp._v_attrs.arguments = np.array(['pos'])
    ...
    pair_sign_wall = fields[:,4]
    pair_sign_wall = pair_sign_wall.astype('float')

    create_array(grp, 'id',           obj=pairs)
    create_array(grp, 'equil_dist',   obj=pair_dists)
    create_array(grp, 'spring_const', obj=pair_spring_consts)
    create_array(grp, 'bonded_atoms', obj=np.zeros(len(pairs),dtype='int'))
    create_array(grp, 'signs',        obj=pair_sign_wall)
...
def main():
    ...
    parser.add_argument('--long-hbond-energy', default='',
            help='energy for forming a longer H-O interaction')
    ...
    parser.add_argument('--wall-spring', default='',
            help='Path to text file that defines a wall restraint atom/residue pairs list.  The first line of the file ' +
            'should be a header containing "residue1 residue2 distance spring_constant wall" or '+
            '"atom1 atom2 distance spring_constant wall", and the remaining lines should contain '+
            'space separated values. The last column (wall) shold be 1 or -1, refers to UPPER or '+
    ...
    parser.add_argument('--plumed', default='', help='plumed engine')
    parser.add_argument('--plumed-only-cb', default='', help='just pass the Cb postions to PLUMED')
    parser.add_argument('--print-hbond', default='', help='Give the PLUMED control files for hbond.')
    parser.add_argument('--print-cter-hbond', default='', help='Give the PLUMED control files for C-terminal hbond.')
    parser.add_argument('--print-NH-env', default='', help='Give the PLUMED control files for NH env number.')
    ...
    parser.add_argument('--COM-environment-potential', default='',
            help='Path to many-body environment potential for sc com')

    parser.add_argument('--long-hydrophobic-energy', default=0.0, type=float,
            help='long environment potential energy')
    parser.add_argument('--long-hydrophobic-left', default=8.0, type=float,
            help='long environment potential energy')
    parser.add_argument('--long-hydrophobic-right', default=20.0, type=float,
            help='long environment potential energy')

    parser.add_argument('--reference-state-rama', default='',
            help='Do not use this unless you know what you are doing.')
    parser.add_argument('--amino-environment', default=False, action='store_true',
            help='Calculate the number of side chains around the hydrogen atom on the amino group.')
    parser.add_argument('--amino-sc-env', default=False, action='store_true',
            help='Calculate the number of side chains nearby the hydrogen atom on the amino group.')
    parser.add_argument('--count-cter-hbond', default=False, action='store_true',
            help='Estimated the hydrogen bonds formed by carboxyl groups at the C-terminus.')
    ...
    if args.bb_environment_potential:
        ...
        write_bb_environment(fasta_seq, args.environment_potential, sc_node_name, args.bb_environment_potential)

    if args.COM_environment_potential:
        write_COM_environment(fasta_seq, args.COM_environment_potential)
    ...
    if args.plumed:
        use_cb_pos = False
        write_plumed(n_atom, args.plumed, use_cb_pos)

    if args.plumed_only_cb:
        use_cb_pos = True
        require_backbone_point = True
        write_plumed(n_atom, args.plumed_only_cb, use_cb_pos)

    if args.print_hbond:
        write_plumed_pf(n_atom, args.print_hbond, 'plumedforce_hb1', 'protein_hbond', 7)
    if args.print_cter_hbond:
        write_plumed_pf(n_atom, args.print_cter_hbond, 'plumedforce_hb2', 'cter_hbond', 1)
    if args.print_NH_env:
        write_plumed_pf(n_atom, args.print_NH_env, 'plumedforce_env', 'donor_environment_coverage', 1)
    ...
    if args.wall_spring:
        print
        print "Wall spring defined by {} file".format(args.wall_spring)

        make_wall_spring(parser, args.wall_spring)
