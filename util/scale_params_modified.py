...
def apply_param_scale(config_fn, hb_scale=1., env_scale=1., rot_scale=1., mempot_scale=1.):
    with tb.open_file(config_fn, 'a') as t:
        pot_group = t.root.input.potential
        if hb_scale != 1.:
            pot_group.hbond_energy._v_attrs.protein_hbond_energy *= hb_scale
            # ToDo: what about the hbond_coverage groups -> how are they coupled
            # to the energy? For env it's clear, but not so for hb

        if env_scale != 1.:
            pot_group.nonlinear_coupling_environment.coeff[:] *= rot_scale
